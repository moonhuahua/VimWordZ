...
v::
	if wordformatcopy = 1
	{
		...
		wordselect := QZData( "{text}" )
		wordselectLength := StrLen(wordselect)

		if wordselectLength=0
			Send, ^+{Up}{Home}^+{Down}
		Word_PasteFormat()
	}
	else
    ...
    while true{
        ...
        if (space_up){
            ...
            if (UserInput == "w")
                ...
                ;~ Send, {Down}
				Word_setFontShrink()
            else if (UserInput == "a")		;ToggleToolbars切换显示工具栏
                ;~ Send, {Left}
                Send, ^!+t
            else if (UserInput == "d")
                ;~ Send, {Right}
				...
				OCR_IBEAM = 32513
				hbeam := DllCall("LoadCursorFromFile","Str", A_ScriptDir "\Lib\zgs.cur")
				DllCall( "SetSystemCursor", Uint,hbeam, Int,OCR_IBEAM )
				
				OutputVar := strlen(ComObjActive("word.application").selection.text)
				if OutputVar=0
					Send, ^+{Up}{Home}^+{Down}   ;^{Up}
				wordformatcopy := 1
				;~ Sleep 150
				...
				Run %a_scriptdir%\Apps\Word\word文档合并工具.ahk
				return
            }else if (UserInput == "Z"){
				Word_Zoom("-10")
				;~ return
